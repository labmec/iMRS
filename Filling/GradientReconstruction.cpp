...
void GetNeigsCenterAndSol(TPZGeoEl *gel,TPZVec<REAL> AllCellSols, TPZVec<std::pair<TPZVec<REAL>,REAL>> &CenterAndSol);
void RunProblem(TPZGeoMesh* gmesh, TPZVec<REAL> &AllCelSols);
...
REAL GradientLimiter(TPZGeoEl *gel, REAL cellSol, TPZVec<REAL> gradCell, TPZVec<std::pair<TPZVec<REAL>,REAL>> &CenterAndSol);
TPZCompMesh* CreateCMesh(TPZGeoMesh* gmesh, std::function<void (const TPZVec<REAL> &, TPZVec<double> &, TPZFMatrix<double> &)>& ExactSolution);

// Definition of the left and right boundary conditions
auto solution = [](const TPZVec<REAL> &coord, TPZVec<STATE> &rhsVal, TPZFMatrix<STATE> &matVal)
{
    REAL x= coord[0];
    REAL y= coord[1];
    REAL z= coord[2];
  ...
  TPZVec<REAL> AllCellSols;
    ...
    for (int i=0 ; i<ngels;i++) {
        ...
        if (geldim==mdim) {
            ...
            TPZVec<std::pair<TPZVec<REAL>, REAL>> CenterAndSol;
...
void RunProblem (TPZGeoMesh* gmesh, TPZVec<REAL> &AllCelAverage){
    int ngels = gmesh->NElements();
    ...
    for (int igel= 0; igel< ngels; igel++) {
        ...
        if (eldim==meshdim) {
            ...
            solution(Xcenter, rhsVal, matVal);
